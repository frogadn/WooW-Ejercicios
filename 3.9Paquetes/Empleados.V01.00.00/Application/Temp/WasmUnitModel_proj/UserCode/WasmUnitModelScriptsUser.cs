...
#region Id
    ...
    public void Nombre_OnChange()
    {
        ...
        string capitalizada = string.Join("", palabras);
        Empleado.Correo = $"{capitalizada}@frog.com.mx";
    }
...
#region FechaDeNacimiento
    ...
    {
        if (Empleado.EstadoCivil == eEmpleado_EstadoCivil.Divorciado)
        {
            EmpleadoControles.Alertas.AgregarAlerta("En esta empresa hay apoyos para los hombres Divorciados, se enviara un correo con la información", eTipoDeAlerta.Mensaje);
        };
    }
...
#region DeleteInfo
    ...
    public void RowVersion_OnChange()
    {
    }

    public void RowVersion_OnFocus()
    {
    }

    public void RowVersion_OnBlur()
    {
    }

    public void btnCustomEjercicio41_OnClick()
    {
        var oEmpleadoCol = Empleado.List(woTarget, new EmpleadoList() { select = "", filter = "EstadoCivil eq 'Divorciado'", orderby = "", top = 0, skip = 0 });
        EmpleadoControles.Alertas.AgregarAlerta($"El total de empleados divorciados es de {oEmpleadoCol.Value.Count.ToString()}", eTipoDeAlerta.Mensaje);
        ;
    }

    public WasmUnitModelScriptsUser(IJSRuntime JS, WasmUnitModelControls EmpleadoControles)
    {
    ...
    public WasmUnitModelControls EmpleadoControles { get; set; }
}
}
